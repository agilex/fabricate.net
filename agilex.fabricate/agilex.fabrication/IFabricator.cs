...
namespace agilex.fabrication
{
    ...
    /// easiest way to do this is to simply inherit from Fabricator<T>.
    /// </summary>
    /// <typeparam name="T">The type you wish to be fabricated</typeparam>
    public interface IFabricator<T> where T : class
    {
        T FabricateInstance();
        ...
        IEnumerable<T> FabricateCollectionOf(int size, Func<int, object[]> collectionConstructorArgs);
        void FabricationRules(Fabricator<T>.FabricatorConfig fabricatorConfig);
    }
}